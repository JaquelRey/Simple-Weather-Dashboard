...
$(document).ready(function () {
  let history = JSON.stringify(localStorage.getItem("city"))
  

  if (history != "null") {
    const searchedHistory = history.split(',')
    console.log(searchedHistory)
    const index = searchedHistory.length - 1
    const last = searchedHistory[index]
    const lastSearch = last.replace(/\"|\[|\]|\\/g, '')

    console.log(`Last searched city: ${lastSearch}`)
    getLatLon(lastSearch)
